...
function length(str) {
  ...
  const parts = splitNumbersAndUnits(str);
  if (parts.length === 1) {
    if (/^[0-9]$/.test(parts[0])) throw new Error(`Length is missing unit: ${str}`);
...
function timeToSecs(number) {
  ...
  switch (parts.length) {
    ...
    case 2:
      ...
      if (isNaN(secs)) throw new Error(`Time format not recognized: ${number}`);

      return secs;

    default:
      throw new Error(`Time format not recognized: ${number}`);
  }
}
